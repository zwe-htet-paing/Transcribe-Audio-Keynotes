...
def json_to_excel(data, filename='audio_keynotes.xlsx'):
    ...
    if data.get("task") == "Transcribe":
        max_len = max(len(value) if isinstance(value, list) else 1 for value in data.values())
        for i in range(max_len):
            row_data = {key: (data[key][i] if isinstance(data[key], list) and i < len(data[key]) else data[key])
                        for key in data}
